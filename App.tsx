...
import { Plus, Search, Sparkles, Settings, Info, X as XIcon, AlertCircle, Loader2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Initial Load - Safely access localStorage
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    try {
      ...
      if (e.name === 'QuotaExceededError') {
        setStorageError("Storage limit reached. Consider removing some photos.");
      }
  ...
  const handleReflect = async (id: string) => {
    ...
    try {
      ...
      setEntries(prev => prev.map(e => 
        e.id === id ? { ...e, aiReflection: reflection, aiImage: aiImage || e.aiImage } : e
      ));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-12 bg-slate-50 text-slate-900 selection:bg-indigo-100 overflow-x-hidden font-sans">
      {storageError && (
        ...
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] uppercase font-black text-indigo-400 tracking-[0.2em] -mt-0.5">Interconnected Memories</p>
            </div>
          </div>
      ...
      <section className="max-w-4xl mx-auto px-4 mt-6 md:mt-10 mb-12">
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-900 rounded-[2.5rem] p-8 md:p-14 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden">
          <div className="relative z-10">
            <span className="inline-block px-3 py-1 bg-indigo-500/30 rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 backdrop-blur-sm border border-indigo-400/30">Your Journey</span>
            <h2 className="text-3xl md:text-5xl font-black mb-6 leading-[1.15]">Capture your unfolding story.</h2>
            <p className="text-indigo-100 text-lg md:text-xl max-w-xl mb-10 leading-relaxed font-medium">
              Every day is a thread. We help you weave them into a beautiful narrative.
            </p>
            
            <div className="flex flex-wrap gap-4">
              <button onClick={handleAddEntry} className="bg-white text-indigo-700 px-8 py-4 rounded-2xl font-black flex items-center gap-2 hover:bg-slate-50 transition-all shadow-xl hover:-translate-y-0.5">
              ...
              >
                ...
                {isGeneratingSummary ? "Synthesizing..." : "Summarize the Thread"}
              </button>
            </div>
          </div>
        ...
        {yearSummary && (
          <div className="mt-8 bg-white border border-indigo-100 p-8 md:p-10 rounded-[2rem] shadow-sm animate-in fade-in slide-in-from-top-4 relative group">
            <button onClick={() => setYearSummary(null)} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-full">
              <XIcon size={20} />
            </button>
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-indigo-50 rounded-lg text-indigo-600"><Sparkles size={24} /></div>
              <h3 className="text-xl font-black text-slate-800 tracking-tight">The Big Picture</h3>
            </div>
        ...
        <div className="relative pb-20">
          {filteredEntries.length > 0 ? (
            ...
            <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-200 shadow-sm">
              ...
              <h3 className="text-2xl font-black text-slate-700 mb-3">Your thread starts here.</h3>
              ...
              <button 
                ...
                className="px-10 py-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-700 transition-all shadow-xl"
              >
      ...
      <button 
        onClick={handleAddEntry} 
        className="md:hidden fixed bottom-8 right-8 w-16 h-16 bg-indigo-600 text-white rounded-full shadow-2xl flex items-center justify-center z-40"
      >
